...
Testimonials with four display layouts, an optional slider on any of them, seven quotation-mark treatments, and a settings screen for colours, fonts and icons.
...
**Seven quotation-mark treatments**
...
* Twin corners - opening and closing marks in two alternating accents, on a bordered card
...
* Fixed: Twin corners' closing mark sat on its own line below the
  attribution instead of beside it. It now sits on the same line as the
  name and role, vertically centred with them, at the right-hand end of
  that row.

= 1.8.1 =
* Changed: Twin corners' closing mark moves back to the bottom-right of
  the card (opening mark stays top-left), restoring the diagonal layout
  the style is named for, while staying ordinary in-flow content so it
  cannot overlap the quote or attribution.
...
* Fixed: Ledger and Drop-cap fusion could render the quotation mark on top
  of the quote text when a theme's own blockquote styling overrode the
  layout this plugin needs for them. Both are now protected against that.
* Added: Border width and border radius, as a site-wide default and as a
  per-block override.
* Changed: The Design screen now shows layout, colours and the quote
  typeface up front; slider timing, glyph tuning, icon sets and other
  less common settings move into collapsed sections lower on the page.

= 1.6.0 =
* Added: Twin corners quotation mark - a bordered card with the opening
  glyph at one corner and its closing counterpart at the diagonally
  opposite one. Neighbouring cards alternate between two accent colours,
  the second set via the new "Accent 2 (Twin corners)" colour on the
  Design screen or as a block-level override.
